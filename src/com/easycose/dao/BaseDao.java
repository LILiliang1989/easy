...
public class BaseDao<T extends Serializable> {
	...
	public List<T> findByPagers(Pager pager,int lastnum, Order order, Property ... propertys) {
	...
	public List<T> findByPagers(Pager pager,int lastnum, Order[] orders, Property ... propertys) {
		Criteria criteria = getSession().createCriteria(clazz);
		//查询条件
		for(Property property:propertys){
			if(property != null)
		...
		if(pager != null){
			...
			{
				int startPage=pager.getCurrentPage()-lastnum/pager.getPageSize();
				int lastSite=lastnum%pager.getPageSize();
				criteria.setFirstResult((startPage - 1) * pager.getPageSize()-lastSite);
				criteria.setMaxResults(pager.getPageSize());
			}
