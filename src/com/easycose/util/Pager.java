...
public class Pager implements Serializable{
	...
	private int showPageNumList_maxLength=7; //showPageNumList的最长长�?
//	private int nearbyPages_realLength=0;    
	...
	public List<Integer> getShowPageNumList() {
		...
		if(this.getTotalPage()<=this.getShowPageNumList_maxLength()){            //1、最多只有showPageNumList_maxLength个页下标，把�?有的页都罗列出来
		...
		else{                                                                    //2、页下标个数超过showPageNumList_maxLength�?				
			if(this.getCurrentPage()-1 < this.getShowPageNumList_maxLength()/2){ //2.1、当前页面前面的页下标个数少于showPageNumList_maxLength的一�?
		...
		//		System.out.println("pager           2.1");
			...
			else if(this.getTotalPage()-this.getCurrentPage()<this.getShowPageNumList_maxLength()/2){ //2.2、当前页面后面的页下标个数少于showPageNumList_maxLength的一�?
				//�?后的	showPageNumList_maxLength�?
		...
		//		System.out.println("pager           2.2");
			...
			else{                                                             //2.3、当前页面前后的页下标个数都少于showPageNumList_maxLength的一�?
